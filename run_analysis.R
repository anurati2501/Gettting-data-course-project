library(dplyr)
library(data.table)
setwd("C:/Data Science/Getting files/getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset")
...
sel_features <- grep(".*mean\\(\\)|std\\(\\)", features[,2], ignore.case = FALSE)
#sel_features
x_total      <- x_total[,sel_features]

# name columns
colnames(x_total)   <- features[sel_features,2]
